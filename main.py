...
def CheckSummary(target_text: str) -> str:
    # divide long text
...
def CheckSentiment(target_text: str) -> Dict[str, float]:
    # divide long text
    sub_texts = GetDividedSubstring(target_text, MAX_TEXT_LENGTH)
...
def CheckNe(target_text: str) -> List[str]:
    # divide long text
    sub_texts = GetDividedSubstring(target_text, MAX_TEXT_LENGTH)
    ...
    for code in codes:
        ...
        if True:
            ...
            if business_policy_environment_issue_etc_similarity < 0.8:
                business_policy_environment_issue_etc = CheckSummary(infoDict[TARGET_YEAR].business_policy_environment_issue_etc_text)
            ...
            if business_risks_similarity < 0.8:
                business_risks = CheckSummary(infoDict[TARGET_YEAR].business_risks_text)
                result_text += '#### 事業等のリスク\n'
                result_text += '<details><summary>## 要約</summary><div>\n{0}\n</div></details>\n'.format(business_risks)

            # 感情分析
            result_text += '#### 財政状態、経営成績及びキャッシュ・フローの状況の分析\n'
            sentiment = CheckSentiment(infoDict[TARGET_YEAR].business_analysis_of_finance_text + infoDict[TARGET_YEAR].business_management_analysis_text)
            ...
            if contradicted:
                ...
                summary = CheckSummary(infoDict[TARGET_YEAR].business_analysis_of_finance_text + infoDict[TARGET_YEAR].business_management_analysis_text)
                result_text += '<details><summary>要約</summary><div>\n{0}\n</div></details>\n'.format(summary)

            # 固有表現抽出
            result_text += '#### 研究開発活動\n'
            ne = CheckNe(infoDict[TARGET_YEAR].business_research_and_development_text)
            result_text += '<details><summary>キーワード</summary><div>\n{0}\n</div></details>\n'.format(ne)
            summary = CheckSummary(infoDict[TARGET_YEAR].business_research_and_development_text)
            result_text += '<details><summary>要約</summary><div>\n{0}\n</div></details>\n'.format(summary)
